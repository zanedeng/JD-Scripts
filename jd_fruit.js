...
function runTimes() {
  return new Promise((resolve, reject) => {
    $.get({
      url: `${$.isNode() ? require('./USER_AGENTS').hwApi : '${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}'}runTimes?activityId=farm&sharecode=${$.farmInfo.farmUserPro.shareCode}`
    }, (err, resp, data) => {
      if (err) {
