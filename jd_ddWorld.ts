    ...
    for (let t of res.result.taskVos) {
      if (t.status === 1) {
        ...
        if (t.taskName === '邀请好友助力') {
          ...
          try {
            // await axios.get('${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}autoInsert/ddworld?sharecode=' + encodeURIComponent(t.assistTaskDetailVo.taskToken + ',' + res.openid))
