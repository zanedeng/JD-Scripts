...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    // 获取随机红包码
    try {
      let {data}: any = await axios.get('${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}jxmchb/20', {timeout: 10000})
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    // 获取随机助力码
    try {
      let {data}: any = await axios.get('${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}jxmc/30', {timeout: 10000})
...
function makeShareCodes(code: string) {
  return new Promise(async (resolve, reject) => {
    ...
    pin = Md5.hashStr(pin)
    await axios.get(`${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}autoInsert/jxmc?sharecode=${code}&bean=${bean}&farm=${farm}&pin=${pin}`, {timeout: 10000})
      .then((res: any) => {
...
function makeShareCodesHb(code: string) {
  return new Promise(async (resolve, reject) => {
    ...
    let pin: string = cookie.match(/pt_pin=([^;]*)/)![1]
    pin = Md5.hashStr(pin)
    await axios.get(`${$.isNode() ? require('./USER_AGENTS').hwApi : 'https://api.jdsharecode.xyz/api/'}autoInsert/jxmchb?sharecode=${code}&bean=${bean}&farm=${farm}&pin=${pin}`, {timeout: 10000})
      .then((res: any) => {
        if (res.data.code === 200)
